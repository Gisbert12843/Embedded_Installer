...
HFONT SG_InputBox::m_hFont_textfield = NULL;
...
LRESULT CALLBACK SG_InputBox::WndProc(HWND hWnd, UINT message, WPARAM wParam, LPARAM lParam)
{
	...
	case WM_CREATE:
		...
		lstrcpyW(lfont.lfFaceName, FONT_NAME);
		...
		memset(&lfont_textfield, 0, sizeof(lfont_textfield));
		lstrcpyW(lfont.lfFaceName, FONT_NAME);
		lfont_textfield.lfHeight = FONT_HEIGHT_TEXTFELD;
		lfont_textfield.lfWeight = FW_NORMAL;//FW_BOLD;
		lfont_textfield.lfItalic = FALSE;
		lfont_textfield.lfCharSet = DEFAULT_CHARSET;
		lfont_textfield.lfOutPrecision = OUT_DEFAULT_PRECIS;
		lfont_textfield.lfClipPrecision = CLIP_DEFAULT_PRECIS;
		lfont_textfield.lfQuality = DEFAULT_QUALITY;
		lfont_textfield.lfPitchAndFamily = DEFAULT_PITCH;
		m_hFont_textfield = CreateFontIndirect(&lfont_textfield);
		// 
		...
		// setting font
		SetFontToControl_textfield(m_hWndPrompt);
		SetFocus(m_hWndEdit);
	...
	case WM_DESTROY:
		...
		DeleteObject(m_hFont_textfield);
